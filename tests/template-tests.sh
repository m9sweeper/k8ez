...
helm template placeholder ../charts/k8ez --values everything.values.yaml --set-string nameOverride=k8sIngress-nameOverride --set-string fullnameOverride=test-everything-app-k8sIngress -n default > full-features-test.yaml

echo "\n\n\n\n----------------------------- RUNNING TESTS WITH MANY CONFIGS AND ISTIO ---------------------------------\n\n\n\n"
helm template placeholder ../charts/k8ez --values everything.values.yaml --set-string nameOverride=ingress-nameOverride --set-string fullnameOverride=test-everything-app-ingress --set-string ingress.k8sIngress.enabled=false --set-string ingress.istio.enabled=true --set-string ingress.istio.forceHttpRedirect=true -n default > full-features-istio-test.yaml
